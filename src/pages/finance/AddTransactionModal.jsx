            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pointer-events-none">
                    ...
                    <motion.div
                        ...
                        className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 shadow-2xl pointer-events-auto relative max-h-[90vh] overflow-y-auto"
                    >
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">
                                {editTransactionId ? 'Edit Entry' : 'New Transaction'}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl">
                                {['expense', 'income', 'transfer'].map(t => (
                                    <button
                                        ...
                                        className={`flex-1 py-3 text-sm font-bold capitalize rounded-xl transition-all ${type === t ? 'bg-white dark:bg-slate-700 shadow-lg text-slate-900 dark:text-white' : 'text-slate-500'}`}
                                    >{t}</button>
                                ))}
                            </div>
                            ...
                            <div className="text-center py-6">
                                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 block mb-2">Amount</label>
                                <div className="flex items-center justify-center gap-2">
                                    ...
                                    <input
                                        ...
                                        className="bg-transparent border-none text-6xl font-black text-slate-900 dark:text-white placeholder:text-slate-100 dark:placeholder:text-slate-800 focus:ring-0 w-full max-w-[240px] text-center"
                                        placeholder="0"
                            ...
                            <div className="flex gap-2 justify-center flex-wrap">
                                {QUICK_AMOUNTS.map(amt => (
                                    <button
                                        ...
                                        className="px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-full text-xs font-bold text-slate-600 dark:text-slate-400 border border-slate-100 dark:border-slate-800 hover:scale-105 transition-all"
                                    >+ ₹{amt}</button>
                                ))}
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {type !== 'transfer' && (
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 font-bold text-slate-900 dark:text-white"
                                        >
                                ...
                                {type === 'transfer' ? (
                                    <>
                                        <div>
                                            ...
                                            <select
                                                ...
                                                className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 font-bold"
                                            >
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Account</label>
                                        <select
                                            value={accountId}
                                            onChange={e => setAccountId(e.target.value)}
                                            className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 font-bold"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 font-bold text-left flex items-center justify-between"
                                    >
                                        <span>{format(new Date(date + 'T00:00:00'), 'MMM d, yyyy')}</span>
                            ...
                            {type === 'expense' && !editTransactionId && (
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-bold border transition-all ${isRecurring ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 border-slate-200 text-slate-500'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-bold border transition-all ${isSplit ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-slate-50 border-slate-200 text-slate-500'}`}
                                    >
                            ...
                            <div className="flex gap-4 pt-4">
                                {editTransactionId && (
                                    <button
                                        ...
                                        className="p-5 bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-2xl hover:bg-rose-100 transition-colors"
                                    ><Trash className="w-5 h-5" /></button>
                                )}
                                <button
                                    type="submit"
                                    className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold py-5 rounded-2xl shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-2"
                                >
                                    {editTransactionId ? 'Update Record' : 'Save Record'}
