...
const EditTaskModal = ({ isOpen, onClose, task }) => {
    ...
    // Populate form when opening or task changes
    useEffect(() => {
        if (isOpen && task) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await updateTask(task.id, {
                title,
                name: title, // Handle legacy 'name' prop
                description,
                category,
                priority,
                time: startTime || null,
                    ...
                    >
                        ...
                        <div className="bg-white dark:bg-[#111827] w-full max-w-sm rounded-[24px] pointer-events-auto flex flex-col shadow-2xl relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/[0.06]">
                                ...
                                <button 
                                    onClick={onClose} 
                                    className="p-1.5 -mr-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-full transition-colors active:scale-95"
                                >
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                            ...
                            <div className="overflow-y-auto max-h-[75vh]">
                                <form id="editTaskForm" onSubmit={handleSubmit} className="p-5 flex flex-col gap-5">
